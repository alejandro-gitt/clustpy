...
def solution_from_move(network, s_iter, node):
    ...
    def create_new_community(communities,node):
        print('Ha elegido crear una comunidad')
        ...
        sol_with_new_community = [frozenset(community_setted) for community_setted in communities_setted]
        print('quedan las siguientes comunidades:',sol_with_new_community)
        return sol_with_new_community
...
def explore_neighborhood(network, s_iter, s_best, tabu_moves, s_neigh, node_best):
    print('BEST SOLUTION QUE ACABA DE ENTRAR A EXPLORE NEIGHBORHOOD',s_best)
    node_best = 0
    for node in network.nodes:
        print('BEST SOLUTION justo antes de  solution from move',s_best)
        s_move = solution_from_move(network, s_iter, node) #¿necesario introducir network?, creo que no (y de momento no se usa)
        ## Añadido por mi 
        ##
        print('BEST SOLUTION tras solution from move',s_best)
...
def tabu_modularity_optimization(network, s_init):
    ...
    '''
    tabu_tenure = 0 # maximo numero de iteraciones en los que un mismo movimiento no se puede repetir
